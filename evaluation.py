...
def evaluate(hp, validloader, model):
    ...
    for valid in validloader:
        ...
        x_, input_length_, y_, _, out_length_, ids_, p_, attn_prior_ = valid

        with torch.no_grad():
            ilens = torch.tensor([x_[-1].shape[0]], dtype=torch.long, device=x_.device)
            olens = torch.tensor([y_[-1].shape[0]], dtype=torch.long, device=x_.device)

            xs_mask = get_mask_from_lengths(ilens)
            ...
            if y_ is not None:
                attn_soft, attn_logprob, attn_hard, attn_hard_dur = self.run_aligner(
                    x_, ilens, xs_mask, y_.permute(0, 2, 1), olens, attn_prior_
                )
            dur_ = attn_hard_dur
            _, after_outs, d_outs, p_outs = model._forward(
                x_.cuda(),
                ilens.cuda(),
                out_length_.cuda(),
                dur_.cuda(),
